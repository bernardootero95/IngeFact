...
export default function DebitNotesListPage() {
  ...
  const handleVerRepresentacion = (nota) =>
    abrirRepresentacion({
      ...
      obtenerPdf: () => obtenerRepresentacionPdfNotaDebito(nota.id),
      navigate,
      rutaPreview: `/debit-notes/${nota.id}/representacion`,
      ...
      <main className="flex-1 min-w-0 flex flex-col md:h-screen md:overflow-hidden">
        <header className="min-h-16 py-2 md:py-0 md:h-16 bg-white border-b border-neutralCustom-100 flex items-center justify-between gap-3 px-4 md:px-8 shrink-0">
          <div>
            ...
            <p className="hidden sm:block text-xs text-neutralCustom-500">Consulta las notas débito emitidas contra tus facturas.</p>
          </div>
        </header>
