...
def test_crear_borrador_compra_inexistente_falla_404(db_session):
    import uuid
    ...
    with pytest.raises(HTTPException) as exc_info:
        service.crear_borrador(empresa.id, _payload(proveedor.id, producto.id, compra_id=uuid.uuid4()))
    assert exc_info.value.status_code == 404
