...
class NotaDebitoService:
    ...
    def obtener_url_xml(self, empresa_id: uuid.UUID, nota_id: uuid.UUID) -> str:
        ...
        if not nota.alegra_debit_note_id:
            raise HTTPException(status.HTTP_409_CONFLICT, "Esta nota débito todavía no fue enviada a la DIAN.")
        try:
    ...
    def enviar_por_correo(
        ...
        """Envio manual de la nota debito aceptada por correo (QR + PDF + XML).
        ...
        if nota.estado != "aceptada":
            raise HTTPException(
                status.HTTP_409_CONFLICT, "Solo se puede enviar por correo una nota debito aceptada."
            )
        ...
        ejecutar_envio_reportando_errores(
            lambda: enviar_nota_por_correo(
                self, nota, tipo="debito", destinatario=correo, email_client=EmailClient()
            ),
            "No se pudo preparar el correo de la nota debito.",
        )
    ...
    def _revertir_consecutivo(self, empresa_id: uuid.UUID, consecutivo: int) -> bool:
        """Mismo criterio que NotaCreditoService._revertir_consecutivo, tipo='debito'."""
        return revertir_consecutivo(
            self.db,
            ConsecutivoNota,
            consecutivo,
            ConsecutivoNota.empresa_id == empresa_id,
            ConsecutivoNota.tipo == "debito",
            empresa_id=empresa_id,
        )
