...
describe("adminEmpresas", () => {
  ...
  it("listEmpresas arma el query string solo con los filtros presentes", async () => {
    ...
    expect(apiRequest).toHaveBeenCalledWith("/api/v1/admin/empresas?estado=activo");
  });

  it("listEmpresas sin filtros no agrega query string", async () => {
    apiRequest.mockResolvedValue([]);
    await listEmpresas();
    expect(apiRequest).toHaveBeenCalledWith("/api/v1/admin/empresas");
  });

  it("getEmpresa pide el detalle por id", async () => {
    apiRequest.mockResolvedValue({ id: "1" });
    await getEmpresa("1");
    expect(apiRequest).toHaveBeenCalledWith("/api/v1/admin/empresas/1");
  });

  it("crearEmpresa hace POST con el payload", async () => {
    apiRequest.mockResolvedValue({ id: "1" });
    await crearEmpresa({ razon_social: "Acme" });
    expect(apiRequest).toHaveBeenCalledWith("/api/v1/admin/empresas", { method: "POST", body: { razon_social: "Acme" } });
  });
  ...
  it("cambiarPlanEmpresa hace PATCH al subrecurso /plan", async () => {
    ...
    expect(apiRequest).toHaveBeenCalledWith("/api/v1/admin/empresas/1/plan", {
      method: "PATCH",
  ...
  it("sincronizarEmpresasAlegra hace POST sin body", async () => {
    ...
    expect(apiRequest).toHaveBeenCalledWith("/api/v1/admin/empresas/sync-alegra", { method: "POST" });
  });
});
